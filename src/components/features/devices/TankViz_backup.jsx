...
export default function TankViz({
  ...
  return (
    <div className={`rounded-2xl border bg-white p-4 shadow-sm ${className}`}>
      ...
      <style>{`
        /* === ANIMACIONES DE VÁLVULA MEJORADAS Y DINÁMICAS === */
        @keyframes valveWiggle {
          0% { transform: translateX(0) rotate(0) scale(1); }
          25% { transform: translateX(10px) rotate(15deg) scale(1.08); }
          50% { transform: translateX(15px) rotate(22deg) scale(1.12); }
          75% { transform: translateX(10px) rotate(15deg) scale(1.08); }
          100%{ transform: translateX(0) rotate(0) scale(1); }
        }

        @keyframes valveOpen {
          0% { transform: translateX(0) rotate(0) scale(1); filter: hue-rotate(0deg); }
          25% { transform: translateX(8px) rotate(12deg) scale(1.05); filter: hue-rotate(10deg); }
          50% { transform: translateX(12px) rotate(18deg) scale(1.08); filter: hue-rotate(20deg); }
          75% { transform: translateX(10px) rotate(15deg) scale(1.06); filter: hue-rotate(15deg); }
          100% { transform: translateX(12px) rotate(20deg) scale(1.03); filter: hue-rotate(0deg); }
        }

        @keyframes valveClose {
          0% { transform: translateX(12px) rotate(20deg) scale(1.03); filter: hue-rotate(0deg); }
          25% { transform: translateX(10px) rotate(15deg) scale(1.06); filter: hue-rotate(-10deg); }
          50% { transform: translateX(8px) rotate(10deg) scale(1.08); filter: hue-rotate(-20deg); }
          75% { transform: translateX(4px) rotate(5deg) scale(1.05); filter: hue-rotate(-10deg); }
          100% { transform: translateX(0) rotate(0) scale(1); filter: hue-rotate(0deg); }
        }
        ...
        @keyframes waterFlow { 
          0% { transform: translateX(-40px) translateY(0); opacity: 0.3; }
          25% { transform: translateX(-20px) translateY(-2px); opacity: 0.7; }
          50% { transform: translateX(0) translateY(-4px); opacity: 0.9; }
          75% { transform: translateX(20px) translateY(-2px); opacity: 0.7; }
          100% { transform: translateX(40px) translateY(0); opacity: 0.3; }
        }
        ...
        /* === NUEVAS ANIMACIONES AVANZADAS DE AGUA === */
        @keyframes waterRise { 
        ...
        @keyframes waterRippleEffect { 
          ...
          100% { transform: scale(1.5) translateY(-6px); opacity: 0; }
        }

        @keyframes waterDrop { 
          0% { transform: translateY(-10px) scale(0.5); opacity: 0.8; }
          50% { transform: translateY(5px) scale(1); opacity: 1; }
          100% { transform: translateY(20px) scale(0.8); opacity: 0.6; }
        }

        @keyframes waterStream { 
          0% { transform: translateY(-20px) scaleY(0.8); opacity: 0.3; }
          50% { transform: translateY(-10px) scaleY(1.1); opacity: 0.7; }
          100% { transform: translateY(0px) scaleY(1); opacity: 0.9; }
        }
        ...
        .valve-open {
          animation: valveOpen 1.2s cubic-bezier(0.34, 1.56, 0.64, 1) forwards,
                     valvePulse 2.5s ease-in-out infinite 1.2s,
                     valveGlow 3s ease-in-out infinite 1.5s;
        }

        .valve-closed {
          animation: valveClose 1s cubic-bezier(0.25, 0.46, 0.45, 0.94) forwards,
                     valvePulseRed 3s ease-in-out infinite 1s,
                     valveBounce 0.5s ease-in-out 1s;
        }

        .valve-wiggle {
          animation: valveWiggle 1s cubic-bezier(0.68, -0.55, 0.265, 1.55) forwards !important,
                     valveRotate 2s linear infinite 1s;
        }
        ...
        /* === NUEVAS CLASES DE ANIMACIÓN AVANZADA === */
        .water-rising { 
          animation: waterRise 2s ease-out forwards;
        }

        .water-splash { 
          animation: waterSplash 1.5s ease-out forwards;
        }

        .water-ripple { 
          animation: waterRippleEffect 2s ease-out forwards;
        }
        ...
        .water-stream { 
          animation: waterStream 2s ease-in-out infinite;
        }

        .water-turbulence { 
          animation: waterTurbulence 4s ease-in-out infinite;
        }

        .water-filling { 
          animation: waterFilling 3s ease-in-out infinite;
        }
        ...
        <svg viewBox={viewBox} className="h-full w-full">
          ...
          <g clipPath={`url(#${clipId})`}>
            ...
            <rect 
              ...
              height={y + h - waterTop} 
              fill={`url(#${gradId})`} 
              className={`water-surface water-level-transition ${valveOpen ? 'water-filling' : ''}`}
            />
            ...
            {/* Efectos de partículas dinámicas cuando la válvula está abierta */}
            {valveOpen && level > 10 && Array.from({ length: 6 }).map((_, i) => {
              const particleX = x + 30 + (i * 25);
              const particleY = waterTop + 20 + (i * 8);
              const delay = (i * 0.3).toFixed(2) + "s";
              
              return (
                <g key={`particle-${i}`} clipPath={`url(#${clipId})`}>
            ...
            {/* Efectos de turbulencia en la superficie cuando la válvula está abierta */}
            {valveOpen && level > 5 && (
              <g transform={`translate(${x}, ${waterTop - 5})`}>
                <rect
                  x="0"
                  y="0"
                  width={w}
  ...
  return (
    <div className={`valve-root ${stateClass} ${className}`}>
      <svg
        ...
        aria-hidden
      >
        {/* Sombra base y gradientes mejorados */}
        <defs>
        ...
        <g className="valve-body" filter="url(#valveGlow)">
          {/* Base exterior con efecto metálico mejorado */}
          <circle 
            cx="60" 
            cy="60" 
            r="52" 
