...
// Utilidad simple para leer/guardar en localStorage
...
// (Opcional) semilla si no hay nada
function seedIfEmpty() {
  ...
  const seed = [
    ...
    },
    {
      ...
      signal: 0,
      lastSeen: new Date(Date.now() - 1000 * 60 * 42).toISOString(), // 42m
    },
    {
      id: "dev-003",
      ...
      signal: 77,
      lastSeen: new Date(Date.now() - 1000 * 60 * 3).toISOString(), // 3m
    },
  ];
  saveDevices(seed);
  return seed;
...
export default function InicioPage() {
  ...
  // Cargar (y sembrar demo si está vacío)
  useEffect(() => {
    setDevices(seedIfEmpty());
  }, []);
      ...
      {devices.length === 0 ? (
        <div className="rounded-2xl border bg-white p-10 text-center">
          ...
          <p className="mt-1 text-sm text-gray-600">
            Cuando conectes tus dispositivos aparecerán aquí.
          </p>
            ...
            >
              ...
              <div className="mt-4 flex items-center justify-end gap-2">
                ...
                <button
                  ...
                  onClick={() => {
                    const next = devices.filter((x) => x.id !== d.id);
                    saveDevices(next);
                    setDevices(next);
                  }}
                >
